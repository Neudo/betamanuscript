import { BODY, CARD, FOREST, INK, INVERSE_FOREGROUND, MONO, MUTED, OXBLOOD, OXBLOOD_TEXT, SERIF, SANS } from "../../../shared/config/design-tokens";
...
function PrioritiesPanel() {
  ...
  return (
    <div className="p-5 grid md:grid-cols-2 gap-5" style={{ minHeight: "380px" }}>
      {/* Repeated issues */}
      <div className="p-4 border" style={{ borderColor: "hsl(var(--ink) / 0.1)", background: CARD }}>
        ...
        <div className="space-y-2.5">
          {repeatedAnnotationIssues.map((r) => (
            <div key={r.tag}>
              ...
              <div className="h-1" style={{ background: "hsl(var(--ink) / 0.08)" }}>
                <div
              ...
              <div className="flex gap-0.5">
                {Array.from({ length: 5 }).map((_, j) => (
                  <div
                    ...
                    style={{ background: j < s.score ? FOREST : "hsl(var(--ink) / 0.1)" }}
                  />
                ))}
              </div>
            </div>
        ...
        <div className="space-y-2">
          {attentionItems.map((a, i) => (
            <div key={i} className="flex items-start gap-2">
              <span className="text-[9px] mt-0.5 flex-shrink-0" style={{ fontFamily: MONO, color: OXBLOOD_TEXT }}>{a.chapter}</span>
      ...
      <div className="md:col-span-2 p-4 border" style={{ borderColor: "hsl(var(--ink) / 0.1)", background: CARD }}>
        <div className="text-[9px] uppercase tracking-widest mb-3" style={{ fontFamily: MONO, color: MUTED }}>
          Reader progress
        </div>
            ...
            <div key={r.name}>
              <div className="flex items-center gap-1.5 mb-1.5">
                <div
                  ...
                  style={{
                    background: r.status === "finished" ? FOREST : r.status === "inactive" ? "hsl(var(--ink) / 0.2)" : OXBLOOD,
              ...
              <div className="h-1 mb-1" style={{ background: "hsl(var(--ink) / 0.08)" }}>
                <div
                  className="h-full"
                  style={{
                    width: `${(r.chapter / r.total) * 100}%`,
                    background: r.status === "finished" ? FOREST : r.status === "inactive" ? "hsl(var(--ink) / 0.25)" : OXBLOOD,
                  }}
                />
              </div>
              <span
