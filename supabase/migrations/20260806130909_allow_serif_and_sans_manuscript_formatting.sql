-- A manuscript may preserve whether an imported run is serif or sans-serif,
-- but never an arbitrary client-provided CSS font family.
...
begin
  ...
  loop
    ...
    loop
      if mark_key not in ('bold', 'italic', 'fontFamily') then
        return false;
      end if;
    end loop;
