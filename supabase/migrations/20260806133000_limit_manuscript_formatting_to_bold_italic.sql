-- The first release deliberately keeps authored formatting to the two
-- conventions readers most often need for narrative meaning: bold and italic.
...
begin
  ...
  loop
    ...
    loop
      if mark_key not in ('bold', 'italic') then
        return false;
      end if;
    end loop;

    if (marks ? 'bold' and marks -> 'bold' <> 'true'::jsonb)
      or (marks ? 'italic' and marks -> 'italic' <> 'true'::jsonb) then
      return false;
    end if;
  end loop;
