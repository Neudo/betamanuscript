-- PostgREST function arguments are generated as non-nullable TypeScript values.
-- Accept empty strings for optional values so the browser can clear them without casts.

drop function public.update_manuscript_settings(uuid, uuid, text, text, public.word_count_band, text[], date, integer, text, text);
...
create function public.update_manuscript_settings(
  ...
  p_title text,
  p_logline text,
  p_estimated_word_count_band text,
  p_genre_slugs text[],
  p_reading_deadline text,
...
declare
  ...
  normalized_word_count_band public.word_count_band := nullif(btrim(p_estimated_word_count_band), '')::public.word_count_band;
  normalized_reading_deadline date := nullif(btrim(p_reading_deadline), '')::date;
...
begin
  ...
  set
    ...
    estimated_word_count_band = normalized_word_count_band
  where manuscript_version.id = p_manuscript_version_id;
  ...
  set
    ...
    reading_deadline = normalized_reading_deadline,
...
revoke all on function public.update_manuscript_settings(uuid, uuid, text, text, text, text[], text, integer, text, text) from public, anon;
grant execute on function public.update_manuscript_settings(uuid, uuid, text, text, text, text[], text, integer, text, text) to authenticated;
